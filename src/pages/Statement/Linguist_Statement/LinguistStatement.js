...
import {
  DropdownExternalUserList,
  CreateFilteredLinguistStatement,
...
const LinguistStatement = () => {
  ...
  const { dropdownExternalUser, linguistStatement } = useSelector(
    (state) => state.root.StatementModule
  );
  ...
  useEffect(() => {
    ...
    if (isEmpty) {
      dispatch(setLinguistStatement([])); // Reset the state if formData is empty
    }
  }, [formData, dispatch]);
  ...
  if ((formData.dueDateFrom && !formData.dueDateTo) || (!formData.dueDateFrom && formData.dueDateTo)) {
    ...
    if(!isAllFieldsEmpty){
      dispatch(CreateFilteredLinguistStatement(filteredPayload));
    }
  };
  ...
  const data = sortedData?.map((item, index) => ({
    ...
    freelance_currency: item?.freelance_currency,
  }));

  const totalAmount = linguistStatement?.reduce((sum, item) => {
    return sum + parseFloat(item.Amount/item.currency_rate || 0);
  }, 0);
  const totalPaidAmount = linguistStatement?.reduce((sum, item) => {
...
export default LinguistStatement;
